...
def main_keyboard():
    ...
    builder.row(
        types.KeyboardButton(text="🍅 Отправить отчет"),
        types.KeyboardButton(text="🍉 Мои успехи")
    )
    builder.row(
        types.KeyboardButton(text="🍇 Статистика"),
        types.KeyboardButton(text="🍌 Мотивация дня"),
        types.KeyboardButton(text="🥑 Тест напоминаний")
    )
...
def report_tasks_keyboard(tasks):
    builder = InlineKeyboardBuilder()
    for task in tasks:
        builder.add(types.InlineKeyboardButton(
            text=f"{task[1][:30]}..." if len(task[1]) > 30 else task[1],
            callback_data=f"report_{task[0]}"
        ))
    return builder.as_markup()
...
async def start(message: types.Message):
    description = (
        ...
        "• <b>🍅 Отправить отчет</b> — выбери задачу и напиши короткий отчет.\n"
...
async def finish_task_creation(message: types.Message, state: FSMContext):
    ...
    await send_first_reminder(message.from_user.id, data['task_text'])
    await message.answer(
        f"Задача добавлена! Я буду напоминать тебе о ней {data['days']} дней подряд.\n"
    ...
    for day in range(data['days']):
        reminders_per_day = 1 if is_weekend(now + timedelta(days=day)) else random.randint(5, 10)
        times = set()
        while len(times) < reminders_per_day:
            rand_minute = random.randint(0, 839)
            remind_hour = 7 + rand_minute // 60
            remind_minute = rand_minute % 60
            times.add((remind_hour, remind_minute))
        for i, (remind_hour, remind_minute) in enumerate(sorted(times)):
...
# --- Мои успехи ---
@dp.message(F.text.in_(["🍉 Мои успехи"]))
async def my_success(message: types.Message):
    ...
    tasks = cursor.fetchall()
    if not tasks:
        await message.answer("За последний месяц у тебя нет задач. Самое время добавить новую!")
        return
...
# --- Отправить отчет ---
@dp.message(F.text.in_(["🍅 Отправить отчет", "Отправить отчет"]))
async def ask_report(message: types.Message, state: FSMContext):
    cursor.execute('SELECT id, task_text FROM tasks WHERE user_id = ? AND status = "active"', (message.from_user.id,))
    tasks = cursor.fetchall()
    if not tasks:
        await message.answer("У тебя нет активных задач для отчета.")
        return
    await message.answer(
        "Выбери задачу, по которой хочешь написать отчет:",
        reply_markup=report_tasks_keyboard(tasks)
    )
    await state.clear()

@dp.callback_query(F.data.startswith("report_"))
async def choose_report_task(callback: types.CallbackQuery, state: FSMContext):
    task_id = int(callback.data.split("_")[1])
    await state.update_data(report_task_id=task_id)
    await callback.message.answer("Напиши отчет по выбранной задаче:")
...
async def test_reminders(message: types.Message):
    ...
    tasks = cursor.fetchall()
    if not tasks:
        await message.answer("У тебя нет активных задач для теста напоминаний.")
        return
    await message.answer("Через 10 секунд ты получишь тестовые напоминания по всем своим задачам.")
    # Гарантируем запуск планировщика
    ...
    for task_id, task_text in tasks:
        scheduler.add_job(
            ...
            args=(message.from_user.id, task_id, task_text),
            id=f"test_remind_{message.from_user.id}_{task_id}_{random.randint(1,1000000)}",
            replace_existing=False
        )

async def send_test_reminder(user_id: int, task_id: int, task_text: str):
    phrase = f"🍆 Тестовое напоминание!\n\nНе забудь про задачу: <b>{task_text}</b>"
    await bot.send_message(
        user_id,
        phrase,
        parse_mode=ParseMode.HTML,
